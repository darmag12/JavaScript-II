...
// Problem 1
// log students names using for each
let studentName = [];
students.forEach( student => {
    studentName.push(`Name: ${student.name}`);
});
console.log(studentName);

// Problem 2
// multiply the students coding streak by 2 in order to find out what the new streak will be after a week
let newStreak = [];
students.map( streak => {
    newStreak.push(`Streak: ${streak.codeStreak * 2}`)
});
console.log(newStreak);

// Problem 3
//use filter to return students with streaks of 10 plus

 let streaksAboveTen = students.filter( aboveTen => {
    if (aboveTen.codeStreak < 10) {
        return aboveTen;
    }
 });
 console.log(streaksAboveTen);